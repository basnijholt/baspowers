...
EOF
    ...
    cat > "$repo/assets/baspowers-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/baspowers/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/baspowers/.fixture-keep"
    cat > "$repo/plugins/baspowers/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/baspowers/.fixture-keep
    git -C "$repo" add plugins/baspowers/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/baspowers/skills/example/agents"

    cat > "$repo/plugins/baspowers/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/baspowers/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/baspowers/.codex-plugin" \
        "$repo/plugins/baspowers/.private-journal" \
        "$repo/plugins/baspowers/assets" \
        "$repo/plugins/baspowers/hooks" \
        "$repo/plugins/baspowers/skills/example/agents" \
        "$repo/plugins/baspowers/skills/example"

    cat > "$repo/plugins/baspowers/.codex-plugin/plugin.json" <<EOF
{
  "name": "baspowers",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/baspowers/assets/baspowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/baspowers/assets/app-icon.png"

    cat > "$repo/plugins/baspowers/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/baspowers/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/baspowers/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/baspowers/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/baspowers/hooks/session-start" "$repo/plugins/baspowers/hooks/session-start-codex" "$repo/plugins/baspowers/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/baspowers/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/baspowers/.codex-plugin/plugin.json \
        plugins/baspowers/assets/app-icon.png \
        plugins/baspowers/assets/baspowers-small.svg \
        plugins/baspowers/hooks/hooks-codex.json \
        plugins/baspowers/hooks/run-hook.cmd \
        plugins/baspowers/hooks/session-start \
        plugins/baspowers/hooks/session-start-codex \
        plugins/baspowers/skills/example/agents/openai.yaml \
        plugins/baspowers/skills/example/SKILL.md \
        plugins/baspowers/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/baspowers/.kimi-plugin" \
        "$repo/plugins/baspowers/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/baspowers/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/baspowers/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/baspowers/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/baspowers/.fixture-keep \
        plugins/baspowers/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/baspowers/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/baspowers/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/baspowers-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (baspowers-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/baspowers-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/baspowers/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/baspowers" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/baspowers-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/baspowers'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/baspowers-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/baspowers-*" "Clean no-op local apply does not create sync branch in destination checkout"
