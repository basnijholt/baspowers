#!/usr/bin/env bash
# Regression check: Baspowers should not route new worktrees through the old
...
ROTOTILL_SPEC="$REPO_ROOT/docs/baspowers/specs/2026-04-06-worktree-rototill-design.md"
ROTOTILL_PLAN="$REPO_ROOT/docs/baspowers/plans/2026-04-06-worktree-rototill.md"
...
assert_not_contains "$USING_SKILL" "~/.config/baspowers/worktrees" "using-git-worktrees does not mention old global path"
...
assert_not_contains "$FINISHING_SKILL" "~/.config/baspowers/worktrees" "finishing-a-development-branch does not treat old global path as owned"
...
assert_not_contains "$ROTOTILL_SPEC" "~/.config/baspowers/worktrees" "rototill spec does not preserve old global path policy"
assert_not_contains "$ROTOTILL_PLAN" "~/.config/baspowers/worktrees" "rototill plan does not preserve old global path policy"
