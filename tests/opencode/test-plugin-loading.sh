#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the baspowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/baspowers.js"
...
skill_count=$(find "$BASPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $BASPOWERS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-baspowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-baspowers skill (required for bootstrap)..."
if [ -f "$BASPOWERS_SKILLS_DIR/using-baspowers/SKILL.md" ]; then
    echo "  [PASS] using-baspowers skill exists"
else
    echo "  [FAIL] using-baspowers skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$BASPOWERS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/baspowers/' "$BASPOWERS_PLUGIN_FILE"; then
