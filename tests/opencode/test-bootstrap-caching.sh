...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$BASPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$BASPOWERS_SKILLS_DIR/using-baspowers/SKILL.md" "$TEST_HOME/using-baspowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$BASPOWERS_PLUGIN_FILE" missing
}
