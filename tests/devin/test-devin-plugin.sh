#!/usr/bin/env bash
# Validate the Devin CLI integration. `devin plugins install obra/baspowers`
...
if manifest.get("name") != "baspowers":
    raise AssertionError(f"plugin name: expected 'baspowers', got {manifest.get('name')!r}")
