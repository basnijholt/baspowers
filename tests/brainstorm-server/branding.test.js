/**
 * Tests for the visual companion's Baspowers/Prime Radiant branding.
 */
...
const ASSET_URL = 'https://primeradiant.com/brand/baspowers-visual-brainstorming-logo.png';
...
function createPackagedServerFixture(version) {
  const root = fs.mkdtempSync(path.join('/tmp', 'baspowers-packaged-server-'));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'baspowers', version }, null, 2)
  );
  return {
    root,
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Baspowers v${version}`),
    'branding text should include dynamic package version'
  );
  assert(
    !html.includes(`Baspowers v${version} by`),
    'branding text should not include "by" when the logo is visible'
  );
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">Baspowers v/.test(html),
    'visible logo should appear before the Baspowers version text'
  );
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant Baspowers v${version}`),
    'disabled telemetry should keep plain text Prime Radiant/Baspowers branding'
  );
}
...
async function main() {
  ...
  await test('packaged Codex plugin reads version from .codex-plugin manifest', async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        assert(!html.includes('Baspowers vunknown'), 'packaged plugin should not fall back to unknown version');
      });
    } finally {
  ...
  await test('BASPOWERS_DISABLE_TELEMETRY=true omits remote image but keeps local branding', async () => {
    const port = 3453;
    const dir = '/tmp/brainstorm-branding-disabled';
    await withServer({ port, dir, env: { BASPOWERS_DISABLE_TELEMETRY: 'true' } }, async () => {
      writeFragment(dir);
      await sleep(300);
      const html = await fetchHtml(port);
  ...
  await test('BASPOWERS_DISABLE_TELEMETRY=yes also omits the remote image on the waiting screen', async () => {
    const port = 3454;
    const dir = '/tmp/brainstorm-branding-disabled-waiting';
    await withServer({ port, dir, env: { BASPOWERS_DISABLE_TELEMETRY: 'yes' } }, async () => {
      const html = await fetchHtml(port);
      assertBrandedFallbackText(html);
      assert(!html.includes(ASSET_URL), 'disabled telemetry should omit the remote image');
    });
