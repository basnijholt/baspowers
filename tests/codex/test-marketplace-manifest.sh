...
assert_equal(marketplace.get("name"), "baspowers-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "baspowers"]
assert_equal(len(matching_plugins), 1, "baspowers plugin entry count")
