...
const BASPOWERS_VERSION = readBaspowersVersion();
const BASPOWERS_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/baspowers-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'BASPOWERS_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const BASPOWERS_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(BASPOWERS_VERSION);
  const text = BASPOWERS_TELEMETRY_DISABLED
    ? 'Prime Radiant Baspowers v' + version
    : 'Baspowers v' + version;
  const logo = BASPOWERS_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + BASPOWERS_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(BASPOWERS_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/basnijholt/baspowers">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
