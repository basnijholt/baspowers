...
To invoke a baspowers skill, use:
...
If `skill_view` cannot find a baspowers skill (it may not appear in the catalog
until the plugin fully registers it), fall back to reading the SKILL.md directly:

```
read_file(path="~/.hermes/plugins/baspowers/skills/<skill-name>/SKILL.md")
```
