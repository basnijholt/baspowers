...
BOOTSTRAP_MARKER = "baspowers:using-baspowers bootstrap for hermes"


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install obra/baspowers`): the plugin
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, "using-baspowers", "SKILL.md")):
            return cand
    raise RuntimeError(
        "baspowers plugin: cannot find the skills/ tree "
        f"(looked at {candidates}). Reinstall with "
        "`hermes plugins install obra/baspowers`."
    )
...
def _build_bootstrap(skills_dir: str) -> str:
    with open(
        os.path.join(skills_dir, "using-baspowers", "SKILL.md"),
        encoding="utf-8",
    ) as f:
        body = _strip_frontmatter(f.read())

    tools_path = os.path.join(
        skills_dir, "using-baspowers", "references", "hermes-tools.md"
    )
    ...
    return (
        ...
        f"{BOOTSTRAP_MARKER}\n\n"
        f"You have baspowers.\n\n"
        f"The using-baspowers skill content is included below and is already "
        f"loaded for this Hermes session. Follow it now. "
        f"Do not try to load using-baspowers again.\n\n"
        f"{body}\n\n"
        f"## Loading Baspowers Skills on Hermes\n\n"
        f"Baspowers skills are registered with Hermes' native skill loader: "
        f'invoke one with `skill_view("baspowers:skill-name")` '
        f'(for example `skill_view("baspowers:brainstorming")`). '
        f"If a namespaced lookup returns 'not found', read the skill file "
        f"directly instead:\n"
        f'`read_file("{skills_dir}/skill-name/SKILL.md")`\n\n'
        f"The baspowers skills directory is: `{skills_dir}`\n\n"
        f"{tool_mapping}\n"
        f"</EXTREMELY_IMPORTANT>"
    )
