...
**Spec:** `docs/baspowers/specs/2026-06-09-sdd-task-scoped-review-dispatch-design.md` — read it before starting. Decisions already settled there: full re-reviews stay; the two review stages stay separate; coordinator keeps model judgment; `requesting-code-review/` stays broad.
...
```
- [code-quality-reviewer-prompt.md](code-quality-reviewer-prompt.md) - Dispatch code quality reviewer subagent
- Final whole-branch review: use baspowers:requesting-code-review's [code-reviewer.md](../requesting-code-review/code-reviewer.md)
```
...
```
- **baspowers:requesting-code-review** - Code review template for reviewer subagents
```

with:

```
- **baspowers:requesting-code-review** - Code review template for the final whole-branch review
```
...
Lives in the `evals/` **submodule** (separate repo, `baspowers-evals`). Work on a branch there; the parent submodule-pointer bump happens at finishing time per `evals/CLAUDE.md`.
...
def scaffold_sdd_quality_defect_plan(workdir: Path) -> None:
    ...
    plans_dir = workdir / "docs" / "baspowers" / "plans"
...
pre() {
    ...
    file-contains 'docs/baspowers/plans/report-plan.md' 'formatAdminReport'
    file-contains 'docs/baspowers/plans/report-plan.md' 'repeat\(40\)'
}

post() {
    skill-called baspowers:subagent-driven-development
    tool-called Agent
...
Run: `grep -n "code-quality-reviewer" skills/using-baspowers/references/antigravity-tools.md skills/using-baspowers/references/gemini-tools.md`
...
- [ ] **Step 1: Baseline (skills as released on dev)** — from the main checkout (`/Users/jesse/git/baspowers/baspowers`, on dev), or any checkout without this branch's changes:

```bash
cd evals
export BASPOWERS_ROOT=/Users/jesse/git/baspowers/baspowers
...
- [ ] **Step 2: After (this branch's skills)** — point `BASPOWERS_ROOT` at this worktree:

```bash
cd evals
export BASPOWERS_ROOT=/Users/jesse/git/baspowers/baspowers/.claude/worktrees/sdd-review-dispatch
uv run quorum run scenarios/sdd-rejects-extra-features --coding-agent claude
uv run quorum run scenarios/sdd-go-fractals --coding-agent claude
uv run quorum run scenarios/sdd-svelte-todo --coding-agent claude
uv run quorum run scenarios/spec-reviewer-catches-planted-flaws --coding-agent claude
...
After all tasks pass: the evals submodule commit needs to land in `baspowers-evals` (PR to its `main`), then this branch bumps the `evals` submodule pointer — per `evals/CLAUDE.md`, the parent bump is part of propagation, not optional. Then use baspowers:finishing-a-development-branch. PRs against baspowers target `dev`.
