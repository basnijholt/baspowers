...
**Files:**
- Create: `docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md`
...
`docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md` with exactly these sections, filled from the artifacts:
...
- **Quote bank** — verbatim, minimum these six (pull more from the reply files if useful):
  ...
  - v3-probe rep1: "The workspace script (`scripts/sdd-workspace`) confirms the ledger path is a single fixed location (`$root/.baspowers/sdd`), not plan-scoped, so it will collide across any two plans run in the same repo."
...
```bash
git add docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md
...
- Produces: `sdd-workspace PLAN_FILE` → prints `<repo-root>/.baspowers/sdd/<plan-basename-without-.md>` (creates it; maintains `<repo-root>/.baspowers/sdd/.gitignore` containing `*`). `task-brief PLAN_FILE N [OUTFILE]` → default OUTFILE `<workspace>/task-<N>-brief.md`. `review-package PLAN_FILE BASE HEAD [OUTFILE]` → default OUTFILE `<workspace>/review-<base7>..<head7>.diff`. Task 3's SKILL.md text names exactly these signatures.
...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.baspowers/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.baspowers/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.baspowers/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.baspowers/sdd/.gitignore" && "$(cat "$repo/.baspowers/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .baspowers/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .baspowers/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.baspowers/sdd/plan-a/task-1-brief.md" ]]; then
    ...
    case "$rp_path" in
        "$repo/.baspowers/sdd/plan-a/review-"*.diff)
    ...
    if [[ "$wt_dir" == "$wt_root/.baspowers/sdd/plan-a" && "$wt_dir" != "$dir_a" ]]; then
...
#
# One directory per plan (.baspowers/sdd/<plan-basename>/) so a follow-up
...
# .gitignore at .baspowers/sdd/ keeps every plan's workspace out of
...
# Default OUTFILE: <repo-root>/.baspowers/sdd/<plan-basename>/task-<N>-brief.md
...
# Default OUTFILE: <repo-root>/.baspowers/sdd/<plan-basename>/review-<base7>..<head7>.diff
...
git commit -m "feat(sdd): plan-scoped workspace — one .baspowers/sdd/<plan> dir per plan

sdd-workspace now requires the plan file and resolves
.baspowers/sdd/<plan-basename>/; task-brief and review-package write
...
- At skill start, check for a ledger:
  `cat "$(git rev-parse --show-toplevel)/.baspowers/sdd/progress.md"`. Tasks listed there
...
- Each plan owns a workspace: at skill start, run this skill's
  ...
  directory (`<repo-root>/.baspowers/sdd/<plan-basename>/`), home to
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its first
  ...
  at the old flat path `.baspowers/sdd/progress.md` — is another plan's
...
Old:
```
    "Dispatch final code reviewer subagent (../requesting-code-review/code-reviewer.md)" -> "Use baspowers:finishing-a-development-branch";
```
New:
```
    ...
    "Final review clean: delete this plan's workspace" -> "Use baspowers:finishing-a-development-branch";
```
...
Old:
```
[Read plan file once: docs/baspowers/plans/feature-plan.md]
[Create todos for all tasks]
```
New:
```
[Read plan file once: docs/baspowers/plans/feature-plan.md]
[Resolve workspace: scripts/sdd-workspace docs/baspowers/plans/feature-plan.md — no ledger inside, fresh start]
[Create todos for all tasks]
```
...
- Create: `docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md`
- Delete: `docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md` (content folds into the results doc)
...
#   LAYOUT:   flat (released layout: .baspowers/sdd/progress.md)
#             scoped (new layout: .baspowers/sdd/<plan-basename>/progress.md,
...
> **For agentic workers:** REQUIRED SUB-SKILL: Use baspowers:subagent-driven-development.
...
case "$scenario/$layout" in
  s1/flat)
    mkdir -p .baspowers/sdd
    plan_a_ledger_lines > .baspowers/sdd/progress.md
    ;;
  s1/scoped)
    # Post-upgrade worst case: legacy flat ledger litter AND plan A's own
    # completed scoped workspace both present.
    mkdir -p .baspowers/sdd/2026-07-01-widget-backend
    printf '*\n' > .baspowers/sdd/.gitignore
    plan_a_ledger_lines > .baspowers/sdd/progress.md
    {
      printf '# SDD ledger — plan: docs/plans/2026-07-01-widget-backend.md\n\n'
      plan_a_ledger_lines
    } > .baspowers/sdd/2026-07-01-widget-backend/progress.md
    ;;
  s2/flat)
    mkdir -p .baspowers/sdd
    plan_b_ledger_lines > .baspowers/sdd/progress.md
    ;;
  s2/scoped)
    mkdir -p .baspowers/sdd/2026-07-06-widget-export
    printf '*\n' > .baspowers/sdd/.gitignore
    {
      printf '# SDD ledger — plan: docs/plans/2026-07-06-widget-export.md\n\n'
      plan_b_ledger_lines
    } > .baspowers/sdd/2026-07-06-widget-export/progress.md
    ;;
  *)
...
Sanity-check one build: `bash "$EVAL_ROOT/make-fixture.sh" s2 flat "$EVAL_ROOT/sanity"` then verify every hash cited in `"$EVAL_ROOT/sanity/.baspowers/sdd/progress.md"` resolves via `git -C "$EVAL_ROOT/sanity" cat-file -e <hash>` and that `git -C "$EVAL_ROOT/sanity" log --format='%an %ad' --date=short` shows two authors across two dates.
...
Create `docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md` with exactly these sections (fill with real data):
...
coarse cost proxy. A rerunnable harness case belongs in baspowers-evals
...
```bash
git rm -q docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md
git add docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md
