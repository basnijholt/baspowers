...
- **Remote serving is a first-class scenario.** Baspowers is general-purpose;
...
- Env var (e.g. `BASPOWERS_VISUAL_COMPANION=off`) the skill is told to read —
  simplest, matches what the issue asks for, lives in `.zshrc`.
- A plugin-settings file (`.claude/baspowers.local.md` frontmatter) — more
...
Today `--project-dir` writes session state to `<project>/.baspowers/brainstorm/`
(`start-server.sh:80-84`) and the skill tells the user to gitignore it
(`visual-companion.md:58`). The ask is a `--state-dir` / `BASPOWERS_STATE_DIR`
