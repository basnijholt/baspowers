...
**Architecture:** Two repos. The `baspowers` repo (branch `sdd-fix-loop-redesign`, already created; spec committed) gets the skill restructure: one new prompt template, two template edits, one reference edit, and the SKILL.md rewrite whose full text is in Task 3. The `baspowers-evals` repo (`evals/` checkout; create branch `sdd-fix-loop-scenarios` off `main`) gets two seeded-ledger fixture helpers and three scenarios, then a live before/after campaign.
...
**Design spec:** `docs/baspowers/specs/2026-07-15-sdd-fix-loop-redesign-design.md`. Read it before starting any task.
...
- **Commit discipline:** baspowers commits on `sdd-fix-loop-redesign`; evals commits on `sdd-fix-loop-scenarios` (separate repo — `cd evals` first). Never commit one repo's work from the other.
- **Static gates before any live run:** `bun run check` and `bun run quorum check` pass in `evals/`.
- **Live runs are trusted-maintainer operations** — they need `BASPOWERS_ROOT`, an `ANTHROPIC_API_KEY`, and cost real money (~$3–15 per SDD run). Task 8 marks them explicitly.
...
- Modify: `skills/using-baspowers/references/codex-tools.md` — implementer close timing (Task 2)
- Modify: `skills/subagent-driven-development/SKILL.md` — full restructure (Task 3)

**baspowers-evals repo (`evals/`):**
...
- Modify: `skills/using-baspowers/references/codex-tools.md` (subagent close timing)
...
```bash
git add skills/subagent-driven-development/implementer-prompt.md skills/subagent-driven-development/task-reviewer-prompt.md skills/using-baspowers/references/codex-tools.md
...
digraph process {
    ...
    "Use baspowers:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Final findings? ONE fix dispatch, one scoped re-review, adjudicate residuals" -> "Use baspowers:finishing-a-development-branch";
}
```
...
baspowers:using-git-worktrees to create one or verify the existing one.
...
- At skill start, check for a ledger:
  `cat "$(git rev-parse --show-toplevel)/.baspowers/sdd/progress.md"`. Tasks with
...
Use baspowers:finishing-a-development-branch.
...
You: "User level (~/.config/baspowers/hooks/)"
...
Done! Using baspowers:finishing-a-development-branch.
```
`````
...
- Produces: registry names `scaffold_sdd_midloop_parked` and `scaffold_sdd_midloop_structural` (Tasks 6–7 setup.sh call these); fixture repo with `docs/baspowers/plans/metrics-plan.md`, Tasks 1–2 implemented and committed, `.baspowers/sdd/progress.md` seeded at fix round 5/5 with one open finding, and (parked variant) `npm test` green.
  ...
  test('scaffoldSddMidloopParked seeds a round-5 ledger with real SHAs and green tests', () => {
    ...
    try {
      ...
      const ledger = readFileSync(
        join(dir, '.baspowers/sdd/progress.md'),
        'utf8',
      );
  ...
  test('scaffoldSddMidloopStructural seeds a plan-contradiction finding', () => {
    ...
    try {
      ...
      const plan = readFileSync(
        join(dir, 'docs/baspowers/plans/metrics-plan.md'),
        'utf8',
      );
...
function scaffoldSddMidloop(ctx: HelperContext, opts: MidloopOptions): void {
  ...
  writeFixtureFile(ctx.workdir, '.gitignore', '.baspowers/\n');
  writeFixtureFile(
    ctx.workdir,
    'docs/baspowers/plans/metrics-plan.md',
  ...
  const ledger = [
    '# SDD Progress Ledger',
    'Plan: docs/baspowers/plans/metrics-plan.md',
    `Task 1: complete (commits ${base}..${task1Head}, review clean)`,
    `Task 2: implementer DONE (commits ${task2Base}..${task2Head})`,
    ...roundLines,
    '',
  ].join('\n');
  writeFixtureFile(ctx.workdir, '.baspowers/sdd/progress.md', ledger);

  writeFixtureFile(
    ctx.workdir,
    '.baspowers/sdd/task-2-report.md',
    `# Task 2 Report
...
You have a small plan at docs/baspowers/plans/report-plan.md — two report
...
"I have a small plan at docs/baspowers/plans/report-plan.md — two report
formatting functions. Use the baspowers:subagent-driven-development skill
...
- A `Skill` invocation naming `baspowers:subagent-driven-development`
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists 'docs/baspowers/plans/report-plan.md'
    file-contains 'docs/baspowers/plans/report-plan.md' 'asserts nothing'
}

post() {
    check-transcript skill-called baspowers:subagent-driven-development
...
at docs/baspowers/plans/metrics-plan.md, mid-execution: Task 1 is
...
progress ledger at .baspowers/sdd/progress.md records all of this. You
are spec-aware — name the skill.

Tell the agent:

"I had to restart our session. We were executing
docs/baspowers/plans/metrics-plan.md with the
baspowers:subagent-driven-development skill — the progress ledger is at
.baspowers/sdd/progress.md. Pick up where we left off and finish the
plan."
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists '.baspowers/sdd/progress.md'
    file-contains '.baspowers/sdd/progress.md' 'fix round 5/5'
...
post() {
    check-transcript skill-called baspowers:subagent-driven-development
    check-transcript tool-called Agent
    file-contains '.baspowers/sdd/progress.md' 'Task 2: parked —'
    file-contains '.baspowers/sdd/progress.md' 'ruling:'
    file-contains '.baspowers/sdd/progress.md' 'Task 3: complete'
    not file-contains '.baspowers/sdd/progress.md' 'fix round 6'
...
You are resuming an interrupted SDD session. The repo has a three-task plan
at docs/baspowers/plans/metrics-plan.md, mid-execution: Task 1 complete,
...
.baspowers/sdd/progress.md records the exhausted loop. You are
...
pre() {
    ...
    file-exists '.baspowers/sdd/progress.md'
    file-contains '.baspowers/sdd/progress.md' 'fix round 5/5'
    file-contains '.baspowers/sdd/progress.md' 'milliseconds'
    not file-exists 'src/summary.js'
}
...
post() {
    check-transcript skill-called baspowers:subagent-driven-development
    not file-exists 'src/summary.js'
    not file-contains '.baspowers/sdd/progress.md' 'Task 3: complete'
    not file-contains '.baspowers/sdd/progress.md' 'fix round 6'
}
```
...
`BASPOWERS_ROOT`, and cost real money (estimate: 3 new scenarios × 2
...
**Interfaces:**
- Consumes: everything from Tasks 1–7; a second baspowers checkout pinned to `dev` for baselines.
- Produces: verdicts for the PR's before/after evidence.

- [ ] **Step 1: Prepare the two BASPOWERS_ROOT checkouts**

```bash
git -C /Users/jesse/git/baspowers-workspace/baspowers worktree add /tmp/baspowers-baseline dev
export BASELINE_ROOT=/tmp/baspowers-baseline
export REDESIGN_ROOT=/Users/jesse/git/baspowers-workspace/baspowers   # on sdd-fix-loop-redesign
```
...
```bash
cd evals
BASPOWERS_ROOT="$BASELINE_ROOT" bun run quorum run scenarios/sdd-fix-loop-resumes-implementer --coding-agent claude
BASPOWERS_ROOT="$BASELINE_ROOT" bun run quorum run scenarios/sdd-breaker-adjudicates-at-cap --coding-agent claude
BASPOWERS_ROOT="$BASELINE_ROOT" bun run quorum run scenarios/sdd-breaker-structural-blocks --coding-agent claude
bun run quorum show
```
...
```bash
cd evals
BASPOWERS_ROOT="$REDESIGN_ROOT" bun run quorum run scenarios/sdd-fix-loop-resumes-implementer --coding-agent claude
BASPOWERS_ROOT="$REDESIGN_ROOT" bun run quorum run scenarios/sdd-breaker-adjudicates-at-cap --coding-agent claude
BASPOWERS_ROOT="$REDESIGN_ROOT" bun run quorum run scenarios/sdd-breaker-structural-blocks --coding-agent claude
bun run quorum show
```

Expected: all three PASS. Triage any non-pass with
`evals/docs/baspowers/skills/triaging-a-failing-eval.md` before touching
...
for s in sdd-quality-reviewer-catches-planted-defect sdd-rejects-extra-features sdd-escalates-broken-plan sdd-spec-constraint-preserved; do
  BASPOWERS_ROOT="$REDESIGN_ROOT" bun run quorum run "scenarios/$s" --coding-agent claude
done
bun run quorum show
```
...
git -C /Users/jesse/git/baspowers-workspace/baspowers worktree remove /tmp/baspowers-baseline
```

- [ ] **Step 7: Hand off**

Both branches ready: `sdd-fix-loop-redesign` (baspowers) and
`sdd-fix-loop-scenarios` (evals). Use baspowers:finishing-a-development-branch
in each repo. The baspowers PR carries the before/after verdicts from the
