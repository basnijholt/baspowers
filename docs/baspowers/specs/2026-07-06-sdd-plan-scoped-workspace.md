...
SDD's durable-progress workspace (`.baspowers/sdd/`, introduced v6.0.0/v6.0.3) has
...
The workspace becomes `.baspowers/sdd/<plan-slug>/`, where `<plan-slug>` is
...
- `sdd-workspace PLAN_FILE` — resolves and creates
  `<repo-root>/.baspowers/sdd/<plan-slug>/`, maintains the self-ignoring
  `.gitignore` at `.baspowers/sdd/.gitignore` (parent level, content `*`),
...
```
# SDD ledger — plan: docs/baspowers/plans/<plan-file>.md
```
...
`baspowers:finishing-a-development-branch` — the controller deletes its
...
`WAVE1-HANDOFF.md`) live directly under `.baspowers/sdd/` untouched by any
plan's cleanup.
...
- No git-level guards against committing `.baspowers/` beyond the existing
  parent `.gitignore`.
...
- `sdd-workspace PLAN` prints `<root>/.baspowers/sdd/<slug>` and creates it;
...
Results land in `docs/baspowers/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md`
