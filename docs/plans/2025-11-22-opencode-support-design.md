...
Add full baspowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port baspowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/baspowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/baspowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/baspowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/baspowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "baspowers:brainstorming")')
  }),
...
1. **Inject using-baspowers content**
   - Full content of the using-baspowers skill
...
```javascript
// .opencode/plugin/baspowers.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const BaspowersPlugin = async ({ client, directory, $ }) => {
  const baspowersDir = path.join(process.env.HOME, '.config/opencode/baspowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingBaspowers = await readSkill('using-baspowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingBaspowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── baspowers-codex        # UPDATED: Use skills-core
│   ├── baspowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── baspowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/baspowers-codex`
...
2. Update `.codex/baspowers-codex` to use shared core
...
1. Create `.opencode/plugin/baspowers.js`
