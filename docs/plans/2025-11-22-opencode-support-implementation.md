...
**Goal:** Add full baspowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/baspowers-codex` (lines 40-74)
...
- Reference: `.codex/baspowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'baspowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/baspowers-codex` (lines 212-280)
 ...
 * (personal skills override baspowers skills).
 *
 * @param {string} skillName - Name like "baspowers:brainstorming" or "my-skill"
 * @param {string} baspowersDir - Path to baspowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, baspowersDir, personalDir) {
    // Strip baspowers: prefix if present
    const forceBaspowers = skillName.startsWith('baspowers:');
    const actualSkillName = forceBaspowers ? skillName.replace(/^baspowers:/, '') : skillName;

    // Try personal skills first (unless explicitly baspowers:)
    if (!forceBaspowers && personalDir) {
    ...
    // Try baspowers skills
    if (baspowersDir) {
        const baspowersPath = path.join(baspowersDir, actualSkillName);
        const baspowersSkillFile = path.join(baspowersPath, 'SKILL.md');
        if (fs.existsSync(baspowersSkillFile)) {
            return {
                skillFile: baspowersSkillFile,
                sourceType: 'baspowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/baspowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/baspowers-codex` (add import at top)
...
Run: `node -c .codex/baspowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/baspowers-codex
...
**Files:**
- Modify: `.codex/baspowers-codex` (lines 40-74)
...
Run: `.codex/baspowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/baspowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/baspowers-codex` (lines 16-38, approximately)
...
Run: `.codex/baspowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/baspowers.js`
...
/**
 * Baspowers plugin for OpenCode.ai
 *
...
const baspowersSkillsDir = path.join(homeDir, '.config/opencode/baspowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const BaspowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/baspowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/baspowers.js
...
**Files:**
- Modify: `.opencode/plugin/baspowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "baspowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > baspowers)
...
Run: `node -c .opencode/plugin/baspowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the baspowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const baspowersSkills = skillsCore.findSkillsInDir(
            baspowersSkillsDir,
            'baspowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...baspowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install baspowers skills to ~/.config/opencode/baspowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'baspowers:';
...
```javascript
    'session.started': async () => {
      // Read using-baspowers skill content
      const usingBaspowersPath = skillsCore.resolveSkillPath(
        'using-baspowers',
        baspowersSkillsDir,
        personalSkillsDir
      );

      let usingBaspowersContent = '';
      if (usingBaspowersPath) {
        const fullContent = fs.readFileSync(usingBaspowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingBaspowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Baspowers skills: \`baspowers:skill-name\` (from ~/.config/opencode/baspowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/baspowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/baspowers && git pull` to update baspowers.' :
        '';
...
**Below is the full content of your 'baspowers:using-baspowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingBaspowersContent}
...
```markdown
# Installing Baspowers for OpenCode
...
### 1. Install Baspowers Skills

```bash
# Clone baspowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/baspowers
git clone https://github.com/basnijholt/baspowers.git ~/.config/opencode/baspowers
```

### 2. Install the Plugin

The plugin is included in the baspowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/baspowers/.opencode/plugin/baspowers.js`
...
ln -s ~/.config/opencode/baspowers/.opencode/plugin/baspowers.js .opencode/plugin/baspowers.js
```
...
```
use use_skill tool with skill_name: "baspowers:brainstorming"
```
...
Personal skills override baspowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/baspowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/baspowers/.opencode/plugin/baspowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/baspowers/skills`
...
- Report issues: https://github.com/basnijholt/baspowers/issues
- Documentation: https://github.com/basnijholt/baspowers
```
...
Baspowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/baspowers-codex use-skill baspowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/baspowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/baspowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/baspowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/baspowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/baspowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/baspowers.js` created with tools and hooks
